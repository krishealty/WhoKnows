...
# then run whoknows on a URL, it will automatically make the country-ips.dat file


def startup
# ok, set up rfile. open once.
	whoknows_folder = File.expand_path(File.dirname(__FILE__))	
	country_db = whoknows_folder + "/country-ips.dat"

	if File.exist?(country_db)
		rfile = File.open(country_db, "rb")
	else		
		if File.exist?(whoknows_folder + "/IpToCountry.csv")
			# pack that file & do it once
			last_start = nil
			last_end = nil
			last_country = nil
			File.open(whoknows_folder + "/country-ips.dat","wb") do |wfile|
				IO.foreach(whoknows_folder + "/IpToCountry.csv") do |line|
					next if line !~ /^"/
	...
	f = whoknows_folder + "/country-codes.txt"
	ccnames = {}
