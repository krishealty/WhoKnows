...
class PluginSupport
  ...
  def self.load_plugin(f)
    ...
    if err.message =~ /wrong number of arguments \(given 1, expected 0\)/
      ...
      error("Error loading plugin #{f}. This plugin may be using a deprecated plugin format for WhoKnows version < 0.5.0. Error message: #{err.message}")
    end
  ...
  def self.plugin_list
    ...
    puts
    puts 'Hint:'
    puts 'For complete plugin descriptions use : whoknows --info-plugins <SEARCH>'
