...
class WhoKnowsTest < Minitest::Test
  ...
  def test_version
    p = IO.popen(['./whoknows', '--version'], 'r+')
    res = p.read.to_s
    p.close
    assert res
    assert res =~ %r{^WhoKnows version ([\d\.]+(-dev)?) \( https://morningstarsecurity.com/research/whoknows/ \)$}
  end

  def test_usage
    p = IO.popen(['./whoknows', '--help'], 'r+')
    res = p.read.to_s
    p.close
    assert res
    assert res =~ /^WhoKnows - Next generation web scanner version ([\d\.]+(-dev)?)\.$/
  end

  def test_invalid_url
    res = IO.popen(['./whoknows','--color', 'never', 'https://'], :err => [ :child, :out ]).read
  ...
  def test_verbose_output
    ...
    p.close
    assert res
    assert res.split("\n").flatten.first =~ %r{WhoKnows report for https://#{@test_host}/$}
  end
  ...
  def test_plugins
    ...
    p.close
    assert res
    assert res =~ %r{^https://#{@test_host}/ \[200 OK\] HTTPServer\[Apache\], Title\[WhoKnows - Next generation web scanner.\]$}
  end

  def test_custom_plugin
    p = IO.popen(['./whoknows',
      '--color', 'never',
      '--custom-plugin', ':text=>"WhoKnows - Next generation web scanner."',
      "https://#{@test_host}/"], 'r+')
    res = p.read.to_s
    p.close
    assert res
  ...
  def test_url_prefix
    ...
    p.close
    assert res
    assert_match %r{WhoKnows - Next generation web scanner.}, res
  end
